...
/*hold on key "S2",it will set the board to AP mode -"HANKERILA-A6"
input IP 192.168.4.1 ,it will enter to config wifi .
After the configuration is successful, restart A6 and enter the new IP address(SSD1306 will display) again. The control page is displayed */
#include <WiFi.h>
#include <WebServer.h>
...
const char *apSsid = "HANKERILA-A6";
...
void handleRelayControl() {
  ...
  for (int i = 1; i <= 6; i++) {
    bool state = pcf8574.digitalRead(i - 1);
...
void handleRelay() {
  ...
  if (relayId == "allon") {
    ...
    if (relayNum >= 1 && relayNum <= 6) {
      bool state = pcf8574.digitalRead(relayNum - 1);
      pcf8574.digitalWrite(relayNum - 1, !state); 
    }
  }
...
void setup() {
  ...
  Wire.begin(4, 15); // GPIO4(SDA) GPIO15(SCL)
  for (int i = 0; i < 6; i++) {
